...
-- En PostgreSQL, la codificación y colación se suelen heredar del clúster.
-- Puedes crearla de forma simple:
CREATE DATABASE dwes;

-- O especificar la codificación (ej. UTF8) y colación (ej. español):
-- CREATE DATABASE dwes ENCODING 'UTF8' LC_COLLATE='es_ES.UTF-8' LC_CTYPE='es_ES.UTF-8';

-- IMPORTANTE:
-- Después de crear la base de datos, DEBES CONECTARTE a ella
-- antes de ejecutar el resto del script.
-- En psql, usarías: \c dwes


-- 2. CREAMOS LAS TABLAS

-- Cambiamos INT NOT NULL AUTO_INCREMENT PRIMARY KEY por SERIAL PRIMARY KEY
-- Quitamos ENGINE = INNODB
CREATE TABLE tienda (
...
-- Quitamos ENGINE = INNODB
-- Quitamos la definición de INDEX (familia) de aquí
...
-- Creamos el índice para 'familia' que quitamos del CREATE TABLE
...
-- 3. CREAMOS LAS CLAVES FORÁNEAS
-- Esta sintaxis es estándar de SQL y funciona igual en PostgreSQL
...
-- 4. CREAMOS EL USUARIO (ROL)
-- La sintaxis de MySQL 'CREATE USER ... IDENTIFIED BY ...'
-- se traduce a 'CREATE ROLE ... WITH LOGIN PASSWORD ...'
CREATE ROLE ut3 WITH LOGIN PASSWORD 'ut3';

-- 5. ASIGNAMOS PERMISOS
-- La sintaxis 'GRANT ALL ON dwes.*' de MySQL es diferente en PostgreSQL
-- Se asume que se usa el esquema 'public' por defecto.
GRANT CONNECT ON DATABASE dwes TO ut3;
GRANT USAGE, CREATE ON SCHEMA public TO ut3;
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO ut3;
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO ut3;

-- Opcional: Dar permisos para futuras tablas/secuencias que cree el superusuario
-- ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON TABLES TO ut3;
-- ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON SEQUENCES TO ut3;


-- 6. INSERTAMOS LOS DATOS
...
-- Usamos E'...' para las cadenas que contienen secuencias de escape como \r\n
